...
//     async verifyEmailOrPhoneOtp(payload: IEmailOrPhoneOtpVerification): Promise<IVerificationResponse> {
//         // Input validation
//         if (!payload.oneTimeCode) {
//             throw new ApiError(StatusCodes.BAD_REQUEST, 'OTP is required');
//         }
...
//     async forgetPassword(payload: IForgetPassword): Promise<string> {
//         const { email, phone } = payload;
...
//         // Send OTP
//         await this.verificationHelper.sendOtpToEmailOrPhone({
//             name: user.name,
//             ...(email && { email } || { phone }),
//             type: 'resetPassword',
//         });
...
//     async resetPassword(payload: IResetPassword): Promise<string> {
//         const { email, phone, newPassword, confirmPassword } = payload;
...
//         }
//         if (!verification || verification.resetPassword !== true) {
//             throw new ApiError(StatusCodes.BAD_REQUEST, "You don't have permission to reset the password. Please try again with forget password.");
//         }
