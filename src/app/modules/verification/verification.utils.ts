...
//     private async handleRateLimitingAndRestriction(verification: Document & IVerification, now: Date): Promise<void> {
//         const timeWindow = new Date(now.getTime() - RATE_LIMIT_WINDOW_MS);
...
//     async sendOtpToEmailOrPhone(values: IEmailOrPhoneVerification): Promise<void> {
//         // Input validation
//         if (!values?.email && !values?.phone) {
//             throw new ApiError(StatusCodes.BAD_REQUEST, 'Either email or phone is required');
//         }
...
//         if (verification) {
//             // Handle rate limiting and restriction
//             await this.handleRateLimitingAndRestriction(verification, now);
//         }

//         // Generate new OTP
//         const oneTimeCode = this.generateOTP();
